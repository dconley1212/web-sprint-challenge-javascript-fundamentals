...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀:
/* The nestedFunction can access the variable internal because of closure. Closure is a principal that allows nested functions to be able to look outside of it's scope for variables that it can't find the value for within it's own scope. Essentially, a function nested in another function can always pull variables outside of it's scope, but functions outside of the nested function can't access the variables/ information in the internal function.*/
...
function animalNames(zooAnimals) {
  //you are creating a new array with for each function
  //start with empty array called displayNames, you will push a string with animal name and scientific name
  const displayNames = [];
...
function lowerCaseNames(zooAnimals) {
  //converting animal names to
  const lowercaseAnimalNames = zooAnimals.map((item) =>
...
function lowPopulationAnimals(zooAnimals) {
  const lowPopulation = zooAnimals.filter((item) => item.population < 5);
...
function USApop(zooAnimals) {
  const totalAnimalPopulation = zooAnimals.reduce((acc, item) => {
